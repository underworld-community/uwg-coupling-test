...
# ======
# 
# This notebook reproduce the model in the 2D verison from Thieulot *et al.* (2014). 
# 
# **Keywords:** Coupled model, 2D
# 
# **References**
...
GEO.rcParams["swarm.particles.per.cell.2D"] = 25
...
outputPath = "op_Ex_Thieulot2D_uwg_zres{:n}".format(zRes)
Model = GEO.Model(elementRes=(xRes,zRes),
                  minCoord=(xmin,zmin),
                  maxCoord=(xmax,zmax),
                  gravity=(0.0, -gravity))
...
uppercrust = Model.add_material(name="UppperCrust", shape=GEO.shapes.Layer2D(top=0.0 * u.kilometer, bottom=-14.0 * u.kilometer))
lowercrust = Model.add_material(name="LowerCrust", shape=GEO.shapes.Layer2D(top=-14.0 * u.kilometer, bottom=-28.0 * u.kilometer))
...
Model.set_velocityBCs(left=[half_rate, None],
                       right=[-half_rate, None],
                       top = [None, None],
                       bottom = [None,0.])
