...
fn_coord = fn.input()
deform_fn = w_m * fn.math.cos(2.*math.pi*fn_coord[0]/Lambda)
...
Model.surfaceProcesses = GEO.surfaceProcesses.Badlands(airIndex=[materialA.index],sedimentIndex=sediment.index,XML="badlands.xml", resolution=2. * u.kilometre, checkpoint_interval=0.3 * u.kiloyear,aspectRatio2d=0.25,surfElevation=surfElevation_fn)
