...
class Badlands_evalfelev(SurfaceProcesses):
    ...
    def solve(self, dt, sigma=0):
        ...
        dt_years = np.around(dimensionalise(dt, u.years).magnitude,4)
        ...
        if self.Model.mesh.dim == 3:
            if rank == 0:
                ...
                # elevation for those points
                known_z = self.badlands_model.elevation / fact
                xs = self.badlands_model.recGrid.rectX / fact
                ys = self.badlands_model.recGrid.rectY / fact
                #                 xs = self.badlands_model.recGrid.rectX / fact
                # ys = self.badlands_model.recGrid.rectY / fact
            ...
            interpolate_z = griddata(points=known_xy,
                                         ...
                                         xi=(xs, xs),
                                         method='nearest')
            nd_coords = np.column_stack((xs,ys, interpolate_z))
